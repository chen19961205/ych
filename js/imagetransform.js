...
ge1doot.transform3D.Triangle.prototype.draw = function () {
	if (this.p0.visible || this.p1.visible || this.p2.visible) {
		...
		this.ctx.fillText('《'+this.texture.title+'》',10, 20);
		this.ctx.font = '12px Arial';
		this.ctx.fillStyle = 'orange';
		this.ctx.fillText(this.texture.content,10,350);
		this.ctx.restore();
	}
	return this.next;
}
/* ===================== image constructor ========================== */
ge1doot.transform3D.Image = function (parent, imgSrc, title, content, lev, callback) {
	this.parent        = parent;
	this.points        = [];
	this.triangles     = [];
	...
	this.texture.title = title;
	this.texture.content=content;
